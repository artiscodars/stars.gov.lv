<!-- Header Section -->
<div class="flex justify-between items-center py-8 mb-6 border-b border-gray-300 ">
    <h1 class="text-3xl font-semibold text-primary">Jaunumi</h1>
    <a href="#" class="text-sm text-primary underline">Skatīt visus jaunumus</a>
</div>

<!-- News Cards -->
<div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 pb-20">
    <!-- Card 1 -->
    <div>
        <div class="bg-gray-300 h-96 mb-4 rounded-md"></div>
        <p class="text-sm text-orange-600 mb-2">02.10.2023</p>
        <h3 class="text-lg font-semibold text-primary">
            Vēl līdz 30. novembrim nodarbinātie var pieteikties kompensācijai par profesionālās kvalifikācijas
            iegūšanu
        </h3>
    </div>

    <!-- Card 2 -->
    <div>
        <div class="bg-gray-300 h-96 mb-4 rounded-md"></div>
        <p class="text-sm text-orange-600 mb-2">02.10.2023</p>
        <h3 class="text-lg font-semibold text-primary">
            Vēl līdz 30. novembrim nodarbinātie var pieteikties kompensācijai par profesionālās kvalifikācijas
            iegūšanu
        </h3>
    </div>

    <!-- Card 3 -->
    <div>
        <div class="bg-gray-300 h-96 mb-4 rounded-md"></div>
        <p class="text-sm text-orange-600 mb-2">02.10.2023</p>
        <h3 class="text-lg font-semibold text-primary">
            Vēl līdz 30. novembrim nodarbinātie var pieteikties kompensācijai par profesionālās kvalifikācijas
            iegūšanu
        </h3>
    </div>

    <div>
        <div class="bg-gray-300 h-96 mb-4 rounded-md"></div>
        <p class="text-sm text-orange-600 mb-2">02.10.2023</p>
        <h3 class="text-lg font-semibold text-primary">
            Vēl līdz 30. novembrim nodarbinātie var pieteikties kompensācijai par profesionālās kvalifikācijas
            iegūšanu
        </h3>
    </div>
</div>